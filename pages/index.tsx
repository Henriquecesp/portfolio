...
function Home({ pinnedItems = [], videos = [] }: Props): JSX.Element {
    const intl = useIntl();
    return (
        <>
            <Head>
            ...
            <NextSeo
                ...
                openGraph={{
                    ...
                    title: WEBSITE_TITLE,
                    description: intl.formatMessage({ id: 'hero.subtitle' }, { br: '|' }),
                    images: [{ url: 'https://henriquecesp.dev/hero.png' }],
            ...
            <motion.main
                ...
                <div className="min-h-screen bg-purple-500 text-gray-100 overflow-x-hidden overflow-y-hidden w-full">
                    <Render Components={ComponentsHome} pinnedItems={pinnedItems} videos={videos} />
                </div>
            </motion.main>
        </>
    );
...
export async function getStaticProps() {
    ...
    const response = await fetch(YOUTUBE_API_URL(YOUTUBE_TOKEN));
    const teste = (await response.json()) as {
        items: VideoProps[];
    };

    const pinnedItems = data.user.pinnedItems.nodes;
    const videos = teste?.items ?? [];
